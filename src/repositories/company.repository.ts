  ...
  implements CompanyContract
{
  ...
  getCompanyWithProducts(companyId: string): Promise<CompanyProductsDto> {
    let queryString =
      'SELECT c.id AS company_id, c.name as company_name, c.address as company_address, p.*';
    queryString += ' FROM companies AS c';
    queryString +=
      ' LEFT JOIN products p on c.id = p.companies_id AND p.deleted_at IS NULL';
    queryString +=
      ' LEFT JOIN product_categories pc on p.product_categories_id = pc.id AND pc.active = 1';
    ...
    return this.connect.execute(queryString, filter).then(result => {
      ...
      result.forEach((row: Product) => {
        ...
        const prod = new Product({
          ...row,
          ...{deletedAt: row.deleted_at, companyId: row.company_id},
        });
        products.push(prod);
      });

      return companyProductsDtoBuilder
        .build(company, {products: products})
        .getObject();
    });
  }
}
